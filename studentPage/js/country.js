...
function populateCountry(obj,defaultCountry) {
  if ( postCountry != '' ) {
...
function pickCountry(defaultCountry) {
  ...
  for (var k = 0; k < countryLineArray.length; k++) {
    lineArray = countryLineArray[k].split(':');
    countryCode  = TrimString(lineArray[0]);
    countryName  = TrimString(lineArray[1]);
...
function initCountry(obj,country) {
  populateCountry(obj,country);
 // populateState();
}
